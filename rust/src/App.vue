...
const loadData = async () => {
  ...
  try {
    // Load Settings
    const s = await invoke('get_paths')
    settings.value = s

    const cfg = await invoke('get_config')
    config.value = cfg
    
    const asg = await invoke('get_assignments')
    ...
    try {
      ...
      const dbs = await invoke('db_list')
    ...
    for (const p of ports) {
      newStatus[p] = await invoke('get_status', p)
    }
    ...
    for (const v of versions.value) {
      vStatus[v] = await invoke('get_version_status', v)
    }
...
const loadCronData = async () => {
    try {
        const data = await invoke('cron_get_data')
        cronData.value = data
    } catch(e) {
...
const handleSaveCronJob = async (job: any) => {
    ...
    try {
        ...
        await invoke('cron_save_job', JSON.parse(JSON.stringify(job)))
        showCronJobModal.value = false
        cronSearchQuery.value = ''
        await loadCronData()
        
        // Auto apply
        const res = await invoke('cron_apply')
...
const handleDeleteCronJob = async (id: number) => {
    ...
    loading.value = true
    try {
        await invoke('cron_delete_job', id)
        await loadCronData()
        await invoke('cron_apply')
...
const handleSaveCronEnv = async (env: any) => {
    loading.value = true
    try {
        await invoke('cron_save_env', JSON.parse(JSON.stringify(env)))
...
const handleDeleteCronEnv = async (id: number) => {
    ...
    loading.value = true
    try {
        await invoke('cron_delete_env', id)
        await loadCronData()
        await invoke('cron_apply')
    } catch(e: any) { alert(e.message) } finally { loading.value = false }
}
...
const handleSaveCronWrapper = async (wrapper: any) => {
    loading.value = true
    try {
        await invoke('cron_save_wrapper', JSON.parse(JSON.stringify(wrapper)))
...
const handleDeleteCronWrapper = async (id: number) => {
    ...
    loading.value = true
    try {
        await invoke('cron_delete_wrapper', id)
        await loadCronData()
        await invoke('cron_apply')
    } catch(e: any) { alert(e.message) } finally { loading.value = false }
}
...
const handleSaveCronSettings = async () => {
    loading.value = true
    try {
        await invoke('cron_save_settings', { cronUser: cronData.value.cronUser })
        await invoke('cron_apply')
...
const handleStart = async (version: string, port: number) => {
  loading.value = true
  try {
    await invoke('start_fpm', { version, port })
    const run = await invoke('get_status', { port })
...
const handleStop = async (version: string, port: number) => {
  loading.value = true
  try {
    await invoke('stop_fpm', { version, port })
    const run = await invoke('get_status', { port })
    status.value = { ...status.value, [port]: run }
  } finally {
    loading.value = false
  ...
  try {
     ...
     if(editingAssignmentIndex.value === -1) {
        ...
        if(status.value[oldAsg.port]) {
           try {
              await invoke('stop_fpm', { version: oldAsg.php_version, port: oldAsg.port })
              shouldRestart = true
           } catch(e) {
     ...
     const ok = await invoke('save_assignments', list)
     if(ok) {
       if(shouldRestart) {
          try {
             await invoke('start_fpm', { version: newAssignment.value.php_version, port: newAssignment.value.port })
          } catch(e: any) {
...
const deleteAssignment = async (index: number) => {
  ...
  try {
     ...
     if(status.value[target.port]) {
        try {
           await invoke('stop_fpm', { version: target.php_version, port: target.port })
        } catch(e) {
...
const toggleDb = async (db: string) => {
   if(expandedDb.value === db) {
      ...
      loading.value = true
      try {
         const tables = await invoke('db_get_tables', db)
...
const checkDocker = async () => {
    ...
    try {
        isDockerRunning.value = await invoke('docker_check')
        if(isDockerRunning.value) {
            ...
            isDockerCliInstalled.value = await invoke('docker_is_installed')
        }
    } finally {
...
const launchDocker = async () => {
    ...
    dockerLoading.value = true
    try {
        const ok = await invoke('docker_launch')
        if(ok) {
...
const listContainers = async () => {
    dockerLoading.value = true
    dockerContainers.value = await invoke('docker_list')
    dockerLoading.value = false
}
...
const handleDockerAction = async (action: string, id: string) => {
    ...
    try {
        await invoke('docker_action', { action, id })
        listContainers()
    } catch(e: any) {
...
const handleStartEnv = async () => {
    loading.value = true
    try {
        await invoke('docker_action', { action: 'start', id: 'myapp_nginx myapp_db' })
        listContainers()
    } catch(e: any) {
...
const handleStopEnv = async () => {
    loading.value = true
    try {
        await invoke('docker_action', { action: 'stop', id: 'myapp_nginx myapp_db' })
        listContainers()
    } catch(e: any) {
        alert('Stop failed: ' + e)
    } finally {
...
const executeSql = async () => {
   ...
   sqlMessage.value = null
   try {
      const res = await invoke('db_query', { database: sqlDbTarget.value, sql: sqlContent.value })
      // Success
...
const handleDropTable = async (db: string, table: string) => {
   ...
   loading.value = true
   try {
      await invoke('db_query', { database: db, sql: `DROP TABLE \`${table}\`` })
      alert('Table dropped')
      // Refresh
      expandedDb.value = null
      await toggleDb(db)
...
const handleImportTable = async (db: string, table: string) => {
   const file = await invoke('open_file_dialog')
   if(!file) return
   ...
   if (ext === 'sql') {
       ...
       loading.value = true
       try {
           await invoke('db_import', { database: db, filePath: file })
       ...
       loading.value = true
       try {
           await invoke('db_import_data', { database: db, table: table, filePath: file, type: ext })
           alert('Import successful')
           // Refresh
           expandedDb.value = null
           await toggleDb(db)
...
const handleShowDDL = async (db: string, table: string) => {
    loading.value = true
    try {
        const res = await invoke('db_query', { database: db, sql: `SHOW CREATE TABLE \`${table}\`` })
        let content = res
...
const handleCreateDb = async () => {
    ...
    loading.value = true
    try {
        await invoke('db_query', { database: 'mysql', sql: `CREATE DATABASE \`${name}\`` })
...
const handleDropDb = async (db: string) => {
    ...
    loading.value = true
    try {
        await invoke('db_query', { database: 'mysql', sql: `DROP DATABASE \`${db}\`` })
...
const handleViewData = async (db: string, table: string) => {
    loading.value = true
    try {
        const res = await invoke('db_get_table_data', { database: db, table })
...
const handleInstall = async () => {
  ...
  currentPage.value = 1
  try {
    const list = await invoke('list_installable')
    installableVersions.value = list
  } catch(e: any) {
    alert('Failed to list installable versions: ' + (e.message || e))
...
const startInstall = async (v: string) => {
  ...
  try {
    await invoke('install_version', v)
...
if(window.ipcRenderer) {
   listen('install:progress', (_: any, line: string) => {
   ...
   listen('setup:status', (_: any, status: string | null) => {
...
const handleUninstall = async (v: string) => {
  ...
  confirmCallback.value = async () => {
     ...
     loading.value = true
     try {
       await invoke('uninstall_version', v)
       loadData()
...
const loadCouchbaseBuckets = async () => {
    loading.value = true
    try {
        const buckets = await invoke('couchbase_get_buckets', {
...
const handleDoCouchbaseUpload = async () => {
    ...
    try {
        ...
        const res = await invoke('couchbase_upload_document', {
...
const saveSettings = async () => {
  loading.value = true
  try {
    const ok = await invoke('update_paths', JSON.parse(JSON.stringify(settings.value)))
    if(ok) {
