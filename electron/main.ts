...
import { existsSync, readFileSync, writeFileSync, createReadStream, mkdirSync, chmodSync } from 'node:fs'
...
async function generateCrontabContent() {
  ...
  // Robust retrieval for sqlite3 -json output
  let cronUser = 'root'
  if (Array.isArray(settings) && settings.length > 0) {
    cronUser = settings[0].value || 'root'
  } else if (settings && typeof settings === 'object' && settings.value) {
    // In case it's not wrapped in an array (though -json usually is)
    cronUser = settings.value
  }
...
if [ ! -f "\${SOURCE_FILE}" ]; then echo "Error: Source file \${SOURCE_FILE} not found."; exit 1; fi
if [ ! -d "/etc/cron.d" ]; then mkdir -p /etc/cron.d; fi
...
echo "Success: Updated \${DEST_FILE}"`
  ...
  try {
    ...
    console.warn('Failed to set script permissions (non-critical):', e)
  }
...
async function runCronQuery(sql: string, params: any[] = []): Promise<any> {
  ...
  try {
    // We base64 encode the SQL to avoid any escaping issues between Windows CMD, WSL, and Bash.
    const sqlBase64 = Buffer.from(finalSql).toString('base64')
    const cmd = `echo "${sqlBase64}" | base64 -d | sqlite3 -json ${dbPath}`
    ...
    // Re-throw if it's a modification query that failed
    throw e
  }
}
  ...
  try {
    ...
    if (!depsInstalled) {
      ...
      try {
        ...
        // Ensure /etc/cron.d exists and cron is enabled
        await runWsl(wsl_distro, 'mkdir -p /etc/cron.d && systemctl enable crond && systemctl start crond', { asRoot: true })

        console.log('Build dependencies and cronie installed.')
      } catch (installErr) {
...
async function runWsl(distro: string, cmd: string, options: { asRoot?: boolean; timeout?: number } | boolean = false): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Construct command without double spaces
    let fullCmd = `wsl -d ${distro}`
    if (userFlag) fullCmd += ` ${userFlag}`
    fullCmd += ` -- bash -c "${escapedCmd}"`
...
ipcMain.handle('cron:apply', async () => {
  ...
  try {
    // 1. Deploy the crontab
    const deployOutput = await runWsl(wsl_distro, `bash ${deployScript}`, true)

    // 2. Ensure crond is running and get status
    // systemctl start will fix it if it's dead, and status will give us diagnostics
    const statusCmd = "systemctl enable crond >/dev/null 2>&1; systemctl start crond >/dev/null 2>&1; systemctl status crond"
    const statusOutput = await runWsl(wsl_distro, statusCmd, true)

    return {
      success: true,
      output: deployOutput,
      serviceStatus: statusOutput
    }
  } catch (e: any) {
    throw new Error(e)
  }
